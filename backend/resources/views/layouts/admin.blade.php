    ...
    <div class="flex flex-1 overflow-hidden">
        ...
        <aside class="hidden md:flex w-64 flex-col border-r border-gray-200 bg-white overflow-y-auto z-10">
            <nav class="flex-1 space-y-1.5 px-4 py-6">
                <!-- Nav Item -->
                <a href="/monitor" class="flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition text-gray-700 hover:bg-gray-50 hover:text-gray-900">
                    <svg class="h-5 w-5 shrink-0 text-emerald-600" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 6.75V15m6-6v8.25m.503 3.446l6-1.9c.614-.194 1.05-.758 1.05-1.4V5.713c0-.903-.955-1.52-1.802-1.22l-6 1.9a1.95 1.95 0 00-1.202 0l-6-1.9A1.95 1.95 0 003 5.713v12.217c0 .642.436 1.206 1.05 1.4l6 1.9a1.95 1.95 0 001.202 0z" />
                    </svg>
                    <span class="font-semibold text-emerald-700">Monitoring Paneli (GIS)</span>
                </a>
                ...
                <a href="/admin/messages" class="flex items-center justify-between rounded-lg px-3 py-2 text-sm font-medium transition {{ request()->is('admin/messages*') ? 'bg-forest-700 text-white' : 'text-gray-700 hover:bg-gray-50 hover:text-gray-900' }}">
                    <div class="flex items-center gap-3">
                        <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    ...
                    @php
                        $unresolvedMessagesCount = \App\Models\SupportMessage::where('is_resolved', false)->count();
                    @endphp
                    @if($unresolvedMessagesCount > 0)
                        <span class="inline-flex h-5 w-5 items-center justify-center rounded-full bg-amber-500 text-[10px] font-bold text-white shadow-sm ring-1 ring-white">
            ...
            <div class="border-t border-gray-150 p-4">
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium text-red-600 hover:bg-red-50 transition">
                        <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
